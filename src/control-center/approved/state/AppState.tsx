...
export type AppStateValue = {
  ...
  recordPayment: (input: { invoiceId: string; amount: number; method: PaymentMethod; receivedAt: number; note: string; processingFee: number; requestId: string; expectedTotal: number }) => Promise<void>
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const recordPayment = useCallback((input: { invoiceId: string; amount: number; method: PaymentMethod; receivedAt: number; note: string; processingFee: number; requestId: string; expectedTotal: number }) => (async () => {
    if (demo.enabled) { const now = new Date(input.receivedAt).toISOString(); demo.updateData((current) => { const invoice = current.invoices.find((row) => row.id === input.invoiceId); if (!invoice) return current; const paymentId = `qa-runtime-payment-${current.payments.length + 1}`; const total = Number(invoice.subtotal_amount ?? invoice.amount - Number(invoice.processing_fee_amount ?? 0)) + input.processingFee; const paid = current.payments.filter(p => p.invoice_id === invoice.id && !p.voided_at).reduce((n,p) => n+Number(p.amount),0); if (input.amount > total-paid) throw new Error('Payment exceeds outstanding balance'); return { ...current, payments: [{ id: paymentId, invoice_id: input.invoiceId, customer_id: invoice.customer_id, amount: input.amount, method: input.method, confirmed_by: 'HUMAN', note: input.note || null, received_at: now, recorded_by: QA_FIXTURE_USER_ID, recorded_at: now, voided_at: null, void_reason: null, voided_by: null }, ...current.payments], invoices: current.invoices.map((row) => row.id === input.invoiceId ? { ...row, amount: total, processing_fee_amount: input.processingFee, status: paid+input.amount>=total ? 'PAID' : row.status, paid_at: paid+input.amount>=total ? new Date().toISOString() : null, payment_claimed_at: null, updated_at: now } : row), financialHistory: [{ id: `qa-runtime-financial-${current.financialHistory.length + 1}`, record_type: 'PAYMENT', record_id: paymentId, event_type: 'RECORDED', reason: 'Confirmed manual amount recorded in demo', before_snapshot: null, after_snapshot: { amount: input.amount }, actor_id: QA_FIXTURE_USER_ID, actor_label: 'Salvador', created_at: now }, ...current.financialHistory] } }); return }
    const paymentId = await recordPaymentRecord(input.invoiceId, input.method, input.note, new Date(input.receivedAt).toISOString(), input.amount, input.processingFee, input.requestId, input.expectedTotal)
    await refresh()
    try {
