...
export function mapLeads(data: ControlData): Lead[] {
  return data.leads.map((row) => {
    ...
    return {
      ...
      createdAt: requiredAt(row.created_at),
      lastActivityAt: Math.max(requiredAt(row.updated_at), latestMessageAt, ...(data.activities??[]).filter(activity => activity.entity_id===row.id || activity.entity_id===quote?.id).map(activity=>requiredAt(activity.created_at))),
  ...
  return data.quotes
    ...
    .map((row) => {
      ...
      return {
        ...
        lastActivityAt: Math.max(requiredAt(row.updated_at),requiredAt(row.created_at),...(data.activities??[]).filter(activity=>activity.entity_id===row.id).map(activity=>requiredAt(activity.created_at))),
...
export function mapInvoices(data: ControlData): Invoice[] {
  return data.invoices.map((row) => ({
    ...
    description: row.description,
    subtotalAmount: row.subtotal_amount == null ? undefined : Number(row.subtotal_amount),
    processingFeeRate: row.processing_fee_rate == null ? undefined : Number(row.processing_fee_rate),
