...
export function SettingsSection() {
  ...
  if (error) return <SetupError error={error} />;
  if (!data) return <Loading />;
  const save = async (fn: () => Promise<unknown>) => { try { await fn(); await refresh(); setSaved(true); setTimeout(() => setSaved(false), 1600); } catch (e) { toast.error(e instanceof Error ? e.message : "Could not save"); } };
