...
describe('sent.DM transport contracts', () => {
  ...
  it('sends from a server function with staff auth, SMS pinning, and provider idempotency', () => {
    ...
    expect(sender.indexOf("service.rpc('enqueue_sms'")).toBeLessThan(sender.lastIndexOf("await dispatchSms("))
    expect(sender.indexOf("service.rpc('queue_staff_sms_test'")).toBeLessThan(sender.indexOf("await dispatchSms("))
    expect(sender.indexOf('const actor = await requireStaff')).toBeLessThan(sender.indexOf("input?.action === 'staff-test'"))
  ...
  it('verifies raw signed webhooks and commits events in a single transaction', () => {
    ...
    expect(webhook).toContain('const jobId = typeof inboundResult?.job_id')
    expect(webhook).toContain('kickCommunications(url, key, { jobId })')
    expect(webhook).toContain('result.data?.duplicate === true')
    expect(webhook).toContain("service.rpc('reserve_inbound_opt_in'")
