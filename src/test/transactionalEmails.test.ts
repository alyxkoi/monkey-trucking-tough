...
describe('transactional workflow safety', () => {
  ...
  it('keeps status finalization after provider acceptance and keeps Resend server-side', () => {
    ...
    expect(edge.indexOf("service.rpc('finalize_customer_email_send'")).toBeLessThan(edge.lastIndexOf('queueInvoiceNotice(service, log.id'))
    expect(edge).toContain("template === 'INVOICE_READY'")
    expect(edge).toContain("Deno.env.get('SENT_DM_FIRST_CONTACT_TEMPLATE_ID')")
