...
describe.sequential('executed PostgreSQL SMS transactions', () => {
  ...
  it('requires real paid/completed records for one-time reviews and permanently disables SMS reactivation',async()=>{
    ...
    try {
      ...
      const [j]=await query("insert into jobs(customer_id,category,status,scheduled_date,address,description,completed_at) values($1,'DRIVEWAY','COMPLETED','2026-07-16','Fixture','Fixture','2026-07-17T10:05:00-05') returning id",[c.customerId])
      const [i]=await query("insert into invoices(invoice_number,customer_id,job_id,amount_source,description,amount,status,paid_at) values('FLOW-I4',$1,$2,'JOB','Fixture',100,'PAID','2026-07-17T10:00:00-05') returning *",[c.customerId,j.id])
      ...
      expect(await rpc('sms_automation_guard',['review-request',c.leadId,guard,'2026-07-17T10:05:00-05'])).toBe(false)
      await query("insert into payments(invoice_id,customer_id,amount,method,confirmed_by,received_at) values($1,$2,100,'ACH','HUMAN','2026-07-17T10:00:00-05')",[i.id,c.customerId])
      expect(await rpc('sms_automation_guard',['review-request',c.leadId,guard,'2026-07-17T10:05:00-05'])).toBe(true)
      const reviewCandidates=await query("select rule_id,due_at from communication_candidates('2026-07-17T10:05:00-05') where subject_id=$1",[i.id])
      expect(reviewCandidates.map(r=>r.rule_id)).toEqual(['review-request'])
      expect(new Date(reviewCandidates[0].due_at).toISOString()).toBe('2026-07-17T15:05:00.000Z')
