...
beforeAll(async()=>{
 ...
 for(const file of ['20260915120000_ai_control_audit','20260915193000_followup_completion','20260915201500_outbound_receipt_reconciliation','20260915220000_ai_lifecycle_actions','20260916120000_approved_change_confirmations','20260916153000_sms_lifecycle_hardening','20260916154000_transactional_quote_recipient'])await db.exec(read(file))
},30000)
...
describe.sequential('executed lifecycle write transactions',()=>{
 ...
 it('keeps a quote recipient separate from customer identity even when another customer uses that email',()=>transaction(async()=>{
   ...
   expect((await query('select email from customers where id=$1',[a.c.id]))[0].email).toBeNull()
