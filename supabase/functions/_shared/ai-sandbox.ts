...
export async function simulateConversation(service: any, input: any, config: AiConfig) {
  ...
  const rows: Record<string, any> = {
    leads: { id:'sandbox', customer_id:'sandbox', human_takeover:Boolean(input.takeover), conversation_revision:1, description:String(input.form ?? '').slice(0,2000) },
    customers: { id:'sandbox',name:'Test customer' },
  ...
  const result = await generateAiDraft(fake,{lead_id:'sandbox'},null,config,{sandbox:true})
  ...
  return {...result, reply, blocked, send_allowed:false, database_changes:false}
}
