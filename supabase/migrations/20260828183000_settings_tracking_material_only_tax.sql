...
-- New and still-editable records use the current material-only rule. Updates to
-- finalized historical snapshots never enter these branches.
create or replace function public.enforce_material_only_tax_snapshot()
returns trigger
language plpgsql set search_path = public, pg_temp as $$
declare
  v_should_apply boolean := tg_op = 'INSERT';
begin
  if tg_op = 'UPDATE' then
    if tg_table_name = 'quotes' then
      v_should_apply := old.status = 'DRAFT' and new.status in ('DRAFT','SENT');
    elsif tg_table_name = 'tickets' then
      -- A saved Ticket is a historical snapshot. Only an explicit correction
      -- that changes pricing inputs/totals receives the current tax rule;
      -- printing, syncing and other metadata updates leave its snapshot alone.
      v_should_apply := old.status = 'saved' and new.status = 'saved' and (
        new.materials_subtotal is distinct from old.materials_subtotal
  ...
  if not v_should_apply then return new; end if;
