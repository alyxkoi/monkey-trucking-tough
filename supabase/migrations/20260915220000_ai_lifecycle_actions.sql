...
-- Customer identity stays on customers; requested delivery is not a booking.
...
-- Existing append-only history is also the request/resolution source. No task
-- table or independent lifecycle is introduced. One event per source/type.
...
begin
 ...
 for a in select jsonb_array_elements_text(coalesce(p_plan->'actions','[]')) loop
   ...
   if a='NEW_WORK' and j.status='COMPLETED' then
     -- Same customer identity, fresh transaction. Old quote/job/history stay intact.
...
-- A single acknowledged handoff uses the same lease, consent and outbox path.
-- The pause is atomic with reservation; only this exact acknowledgment survives
-- that pause. A later customer message, staff reply, opt-out or revision cancels it.
