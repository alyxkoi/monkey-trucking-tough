...
declare l public.leads%rowtype; c public.customers%rowtype; m public.lead_messages%rowtype; evidence public.lead_messages%rowtype;
 ...
 a text; v_email text; v_confirmed_email text; material_result jsonb; route_result jsonb; result jsonb; settings public.app_settings%rowtype;
...
begin
 ...
 evidence:=m;
 if nullif(p_plan#>>'{context,request_source_message_id}','') is not null then
   select * into evidence from public.lead_messages where id=(p_plan#>>'{context,request_source_message_id}')::uuid
     and lead_id=l.id and customer_id=c.id and sender_type='CUSTOMER';
   if evidence.id is null then return jsonb_build_object('status','STALE'); end if;
 end if;
 ...
 before_lead:=jsonb_build_object('delivery_address',l.delivery_address,'requested_text',l.requested_delivery_text,'requested_date',l.requested_delivery_date,'requested_time',l.requested_delivery_time,'quote_confirmed_email',l.quote_confirmed_email,'quote_requested_at',l.quote_requested_at,
   'route_evidence_fingerprint',l.route_evidence_fingerprint,'route_evidence_miles',l.route_evidence_miles);
 ...
 if facts_only then
   if nullif(p_plan->>'name','') is not null then
     if length(p_plan->>'name')>80 or position(lower(p_plan->>'name') in lower(evidence.body))=0 then raise exception 'Name must come from the current customer request'; end if;
   end if;

   -- Contact/profile email is identity data and keeps collision protection.
   v_email:=nullif(lower(trim(p_plan->>'email')),'');
   if v_email is not null then
     if length(v_email)>254 or v_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' or position(v_email in lower(evidence.body))=0 then raise exception 'Email must come from the current customer request'; end if;
     if exists(select 1 from public.customers where id<>c.id and normalized_email=v_email) then
     ...
     update public.customers set email=v_email,normalized_email=v_email,updated_at=now() where id=c.id;
     c.email:=v_email;
   end if;
   ...
   -- A quote recipient is transaction-scoped. It may legitimately be an email
   -- already used by a different customer and never merges or mutates identity.
   v_confirmed_email:=nullif(lower(trim(p_plan->>'confirmed_email')),'');
   if v_confirmed_email is not null then
     if length(v_confirmed_email)>254 or v_confirmed_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$'
       or (position(v_confirmed_email in lower(evidence.body))=0 and v_confirmed_email is distinct from lower(coalesce(c.email,'')))
       then raise exception 'Quote recipient confirmation must come from the current customer request or confirmed profile email'; end if;
     update public.leads set quote_confirmed_email=v_confirmed_email where id=l.id;
   end if;
   if j.id is null and (q.id is null or q.status='DRAFT') and p_plan#>>'{context,pricing,route,status}'='ROUTE_CALCULATED' then
     update public.leads set delivery_address=p_plan#>>'{context,pricing,route,destination}',
       route_evidence_fingerprint=nullif(p_plan#>>'{context,pricing,route,evidence_fingerprint}',''),
       route_evidence_miles=(p_plan#>>'{context,pricing,route,distance_miles}')::numeric,
       route_evidence_place_id=nullif(p_plan#>>'{context,pricing,route,destination_place_id}',''),
       route_evidence_calculated_at=now() where id=l.id;
   end if;
 ...
 values(c.id,'LEAD',l.id,'AI_LIFECYCLE_APPLIED','AI processed customer facts and staff actions','AI',jsonb_build_object(
   ...
   'after',jsonb_build_object('customer',(select jsonb_build_object('name',name,'email',email) from public.customers where id=c.id),
     'lead',(select jsonb_build_object('delivery_address',delivery_address,'requested_text',requested_delivery_text,'requested_date',requested_delivery_date,'requested_time',requested_delivery_time,'quote_confirmed_email',quote_confirmed_email,'quote_requested_at',quote_requested_at,'route_evidence_fingerprint',route_evidence_fingerprint,'route_evidence_miles',route_evidence_miles) from public.leads where id=l.id),
