...
- An email-only answer to the recipient question uses the same deterministic intake path. Live testing reproduced a model timeout on this otherwise unambiguous answer; customer conflict checks, recipient evidence, revision checks and the lifecycle transaction still run before any real send.
